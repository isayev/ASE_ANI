...
try:
   os.environ["ANI_HOME"]
   anipath = os.environ.get('ANI_HOME')
except KeyError:
   print("Please set the environment path to ANI")
   sys.exit(1)

# Get custom defined GPUID for ANI or using 0 by default
gpuid = os.getenv('ANI_GPU', '0')
gpuid = int(gpuid)
#gpuid = 0

# Path to ANI C libraries
sys.path.append(anipath+'/lib')
...
class ANID3(Calculator):
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        energy_ani = nc.energy()[0]

        forces_ani = nc.force()        
        
        self.results['energy'] = conv_au_ev*energy_ani + energy_d3
        self.results['forces'] = conv_au_ev*forces_ani + forces_d3.T
...
class ANI(Calculator):
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        self.results['energy'] = conv_au_ev*energy_ani
        self.results['forces'] = conv_au_ev*forces_ani
        self.results['stress'] = conv_au_ev*stress_ani
        self.results['atomicenergies'] =  nc.aenergies()

class ANIatomic(Calculator):
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        self.results['energy'] = nc.aenergies()
